...
import { DEFAULT_TEMPLATE, InvoiceData, LineItem, TemplateSettings } from '@/types'
...
function templateFromDb(key: string, data: Record<string, unknown>): TemplateSettings {
  return {
    ...
    primaryColor: (data.primaryColor as string) ?? DEFAULT_TEMPLATE.primaryColor,
...
export async function getTemplates(): Promise<TemplateSettings[]> {
  ...
  if (!snap.exists()) {
    ...
    await set(newRef, { ...DEFAULT_TEMPLATE, createdAt: Date.now(), updatedAt: Date.now() })
    const newSnap = await get(newRef)
    return [templateFromDb(newRef.key!, newSnap.val() as Record<string, unknown>)]
  }
...
export async function getTemplate(id: string): Promise<TemplateSettings | null> {
  const snap = await get(ref(db, `templates/${id}`))
...
export async function updateTemplate(id: string, data: Partial<TemplateSettings>): Promise<void> {
  const clean = stripMeta(data as unknown as Record<string, unknown>)
  await update(ref(db, `templates/${id}`), { ...clean, updatedAt: Date.now() })
}
...
export async function updateInvoice(id: string, data: Partial<InvoiceData>): Promise<void> {
  const clean = stripMeta(data as unknown as Record<string, unknown>)
  await update(ref(db, `invoices/${id}`), { ...clean, updatedAt: Date.now() })
}

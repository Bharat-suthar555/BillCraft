...
export default function NewInvoicePage() {
  ...
  const [savedInvoice, setSavedInvoice] = useState<Partial<InvoiceData> | null>(
    null,
  );
  ...
  useEffect(() => {
    getTemplates()
      .then((list) => {
        setTemplates(list);
        if (list.length) setTemplate(list[0]);
      })
  ...
  const handleSave = useCallback(
    async (data: Partial<InvoiceData>) => {
      setSaving(true);
      try {
  ...
  return (
    <div className='tdc-container'>
          ...
          {savedInvoice && (
            <>
              <PDFDownloadButton
                ...
                template={template}
                fileName={`invoice-${savedInvoice.billNo ?? 'draft'}.pdf`}
              />
              <Link
                href='/invoices'
                className='inline-flex items-center gap-1 rounded-md border border-border px-3 py-1.5 text-sm text-foreground hover:bg-accent transition-colors'
              >
      ...
      <div className='flex flex-col gap-6 lg:flex-row lg:items-start'>
        {/* Form */}
        <div
          className={`w-full shrink-0 lg:w-105 xl:w-115 ${activeTab === 'preview' ? 'hidden sm:block' : ''}`}
        >
        ...
        <div
          ref={previewRef}
          className={`flex-1 min-w-0 ${activeTab === 'form' ? 'hidden sm:block' : ''}`}
        >
          <div className='tdc-card'>
            ...
            {!savedInvoice && (
              <p className='mt-2 text-center text-xs text-muted-foreground'>
                Save the invoice to unlock PDF download
              </p>
            )}
          </div>
        </div>

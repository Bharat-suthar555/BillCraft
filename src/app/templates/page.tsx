...
import { createTemplate, deleteTemplate, getTemplates, updateTemplate } from '@/lib/firestore'
...
const DEMO_INVOICE = {
  ...
  lineItems: [
    { ...EMPTY_LINE_ITEM(), description: 'Waterproofing – Roof', size: '40×30', sqft: '1200', rate: '25', amount: 30000 },
    { ...EMPTY_LINE_ITEM(), description: 'Waterproofing – Basement', size: '20×15', sqft: '300', rate: '30', amount: 9000 },
  ],
  total: 39000,
}
...
export default function TemplatesPage() {
  ...
  useEffect(() => {
    getTemplates()
      .then(list => { setTemplates(list); setSelected(list[0] ?? null) })
      .catch(() => toast.error('Failed to load templates. Check Firebase config.'))
  }, [])
  ...
  const handleNew = useCallback(async () => {
    setSaving(true)
    try {
      const t = await createTemplate({ ...DEFAULT_TEMPLATE, name: `Template ${templates.length + 1}` })
      setTemplates(prev => [t, ...prev])
      setSelected(t); setDirty(null)
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      if (active._id) {
        ...
        setTemplates(prev => prev.map(t => t._id === active._id ? active : t))
        setSelected(active); setDirty(null)
      } else {
        const saved = await createTemplate(active)
        setTemplates(prev => [saved, ...prev])
        setSelected(saved); setDirty(null)
      }
  ...
  const handleDuplicate = useCallback(async (t: TemplateSettings) => {
    ...
    try {
      ...
      setSelected(copy); setDirty(null)
    ...
    <div className="tdc-container">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Customize the look of your invoices</p>
        </div>
      ...
      <div className="tdc-card">
        <h2 className="mb-3 text-sm font-semibold text-foreground">
          Saved Templates
          <span className="ml-2 text-xs font-normal text-muted-foreground">({templates.length})</span>
        </h2>

        {templates.length === 0 ? (
          <p className="py-4 text-center text-sm text-muted-foreground">No templates yet — create one above</p>
        ) : (
          <div className="flex flex-wrap gap-3">
            {templates.map(t => (
              <div key={t._id}
                onClick={() => { setSelected(t); setDirty(null) }}
                className={`group relative cursor-pointer rounded-xl border-2 p-3 transition-all ${
                  selected?._id === t._id ? 'border-[#1890ff] shadow-md shadow-[#1890ff]/10' : 'border-border hover:border-muted-foreground/30'
                }`}
                style={{ minWidth: 176 }}
              >
                {selected?._id === t._id && (
                ...
                <div className="mb-2 flex gap-1.5">
                  <span className="h-4 w-8 rounded" style={{ backgroundColor: t.primaryColor }} />
                  <span className="h-4 w-5 rounded" style={{ backgroundColor: t.accentColor }} />
                </div>
                <div className="text-sm font-medium text-foreground truncate max-w-40">{t.name}</div>
                <div className="text-xs text-muted-foreground truncate max-w-40">{t.companyName}</div>
                <div className="mt-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={e => { e.stopPropagation(); handleDuplicate(t) }}
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
          <aside className="w-full shrink-0 lg:w-80 xl:w-96">
            <div className="tdc-card">
              ...
              <TemplateEditor template={active} onChange={t => setDirty(t)} onSave={handleSave} saving={saving} />
            </div>
          </aside>
          ...
          <div ref={previewRef} className="flex-1 min-w-0">
            <div className="tdc-card">
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className="text-xs text-muted-foreground">A4 · {Math.round(scale * 100)}% zoom</span>
              </div>
              <div className="overflow-hidden rounded-lg bg-muted/40 p-4" style={{ minHeight: Math.round(1123 * scale) + 32 }}>
                <div style={{ width: 794 * scale, height: 1123 * scale }}>
                  <InvoicePreview invoice={DEMO_INVOICE} template={active} scale={scale} />
                </div>
              </div>
              <p className="mt-2 text-center text-xs text-muted-foreground">Sample data — real invoices use your customer details</p>
            </div>
          </div>
        </div>
      )}

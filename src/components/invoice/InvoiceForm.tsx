...
import { EMPTY_LINE_ITEM, InvoiceData, LineItem, TemplateSettings } from '@/types'
...
export function InvoiceForm({ template, initialData, onChange, onSave, saving }: Props) {
  const { register, control, watch, handleSubmit, setValue } = useForm<FormValues>({
    defaultValues: {
      ...
      lineItems: initialData?.lineItems?.length ? initialData.lineItems : [EMPTY_LINE_ITEM()],
    },
  })
  ...
  const computeTotal = useCallback((items: LineItem[]) => {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Input id="customerPhone" placeholder="+91 98765 43210" {...register('customerPhone')} />
        </div>
        <div className="space-y-1.5">
          ...
          <table className="w-full text-sm min-w-[500px]">
            <thead>
              <tr className="text-xs font-medium text-gray-500 uppercase">
                ...
                {template.showSize && <th className="pb-2 text-left w-20">Size</th>}
                {template.showSqft && <th className="pb-2 text-left w-16">sq.ft</th>}
                <th className="pb-2 text-left w-20">Rate ({template.currencySymbol})</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {fields.map((field, index) => {
                ...
                return (
                  <tr key={field.id}>
                    <td className="py-1.5 pl-1 text-gray-400">{index + 1}</td>
